...
- **Accommodation Integration (Baba Boutique)**: Set **Baba Boutique House Langkawi** (`6.2855, 99.7290`) in Pantai Tengah as the basecamp resort. Re-routed all daily travel paths to start and end at the hotel (represented by a distinctive Teal pin on the map) and customized check-in descriptions.
...
- **Langkawi Departure & KL Route Mapping (Day 1 & Day 4)**: Adjusted the Day 4 (July 20th) itinerary to stay completely in Langkawi until departure at 16:00 (visiting Pantai Cenang/Tengah cafes like Hidden Langkawi) and removed the redundant Petronas Twin Towers/KLCC visit from the end of Day 4. Added the Kuala Lumpur locations (KLIA, Petronas Twin Towers, Jalan Alor) as interactive markers and mapped the Day 1 travel route directly on the Leaflet map to represent the KL arrival day visually when selecting Day 1.
- **Mobile UI Responsiveness for Route Toggles**: Configured the route toggle button group (`.map-filter-group`) to flow horizontally on narrow viewports using `overflow-x: auto` and `flex-shrink: 0` on its buttons instead of wrapping or shrinking. Added `min-width: 0`, `width: 100%`, and `max-width: 100%` boundaries to all parent container divs (`.map-outer`, `.map-header`, `.map-route-container`) to prevent overall layout stretching on mobile screens.
- **Itinerary Repositioning & Budget Table Integration**: Repositioned the Detailed Itinerary section above the Map & Explorer section in the HTML layout. Added estimated Grab costs (in both RM and KRW) to all relevant transit events in the timeline. Created a comprehensive, mobile-friendly Budget Section at the bottom of the page (excluding lodging) listing estimated expenses and tour booking references/links (e.g., Klook and Panorama Langkawi).
